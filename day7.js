...
let minScore
let minCandidate
for(let candidate = minPos; candidate < maxPos ; candidate ++) {
    let score = calculateScore(candidate, positions)
...
console.log('Min fuel ' + minScore + ' Candidate ' + minCandidate)

function calculateScore(candidate, positions) {
    let score = 0
    for( let pos of positions) {
        let difference = Math.abs(pos - candidate)
        //Part2 
        let cost = difference * (difference + 1) / 2
        score += cost
    }
